...
import com.project.loveindp.controller.admin.AdminController;
...
public class ControllerAspect {
    ...
    @Around("execution(* com.project.loveindp.controller.admin..*.*(..)) && @annotation(org.springframework.web.bind.annotation.RequestMapping)")
    public Object adminControllerBeforeValidation(ProceedingJoinPoint joinPoint) throws Throwable {
        Method method = ((MethodSignature)joinPoint.getSignature()).getMethod();
        AdminPermission adminPermission = method.getAnnotation(AdminPermission.class);
        if(adminPermission == null){
            //公共方法
            Object resultObject = joinPoint.proceed();//执行被切面的方法
            return resultObject;
        }
        //判断当前管理员是否登录
